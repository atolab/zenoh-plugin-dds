...
pub(crate) struct DeadlinesSupervisor<'a> {
    ...
    subscribers: HashMap<String, CallbackSubscriber<'a>>,
    deadlines: Arc<RwLock<HashMap<String, SupervisedDeadline>>>,
}

impl DeadlinesSupervisor<'_> {
    pub(crate) fn new<'a>(zsession: &'a Arc<Session>) -> DeadlinesSupervisor<'a> {
        DeadlinesSupervisor {
            zsession,
    ...
    pub(crate) fn supervise(&mut self, zkey: &str, deadline: Duration) {
        ...
        let sub = self
            .zsession
            .declare_callback_subscriber(&zkey.into(), &sub_info, move |sample| {
                let robot_id = &sample.res_name[..sample.res_name.find('/').unwrap_or(0)];
                let topic_name = &sample.res_name[sample.res_name.rfind('/').unwrap_or(0) + 1..];
